...
interface FormEditarTituloProps {
  id: string;
}

export function FormEditarTitulo({ id }: FormEditarTituloProps) {
  const { editarTitulo, buscarTituloPorId } = useTituloHook();
  ...
  useEffect(() => {
    const fetchTitulo = async () => {
      try {
        const titulo = await buscarTituloPorId(Number(id));
        if (titulo) {
          form.reset({
            nome: titulo.nome,
            ...
            classe: titulo.classe.id.toString(),
            diretor: titulo.diretor.id.toString(),
            atores: titulo.atores.map((ator: any) => ator.id),
          });
        }
      } catch (error) {
